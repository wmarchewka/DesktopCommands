...
def eth_addr(a):
    b = "%x:%x:%x:%x:%x:%x" % struct.unpack("BBBBBB", a)
...
while True:
    ...
    print('Destination MAC : ' + eth_addr(packet[0:6]).upper() + ' Source MAC : ' + eth_addr(packet[6:12]).upper() + ' Protocol : ' + str(
        eth_protocol))
